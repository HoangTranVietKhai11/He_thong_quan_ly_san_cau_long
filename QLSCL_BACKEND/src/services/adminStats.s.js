...
const adminStatsService = {
  ...
  /**
   * Helper to calculate golden hour recommendations
   * @private
   */
  _calculateGoldenHours: (bookings, totalCourts, daysCount = 30) => {
    ...
    bookings.forEach(b => {
      ...
      try {
        ...
        let current = startHr + (startMin || 0) / 60;
        const end = endHr + (endMin || 0) / 60;
        
        while (current < end) {
          const h = Math.floor(current);
          if (h >= 0 && h < 24) {
            const nextHour = h + 1;
            const durationInThisHour = Math.min(nextHour, end) - current;
            hourlyStats[h].totalHours += durationInThisHour;
          }
          current = Math.floor(current + 1);
        }
      } catch (e) {
        console.error('Error processing booking time:', e.message);
      }
    });

    return hourlyStats.map(s => {
      const occupancy = (s.totalHours / (totalCourts * daysCount)) * 100;
      return {
        hour: s.hour,
        occupancy: parseFloat(occupancy.toFixed(2)),
        isRecommended: occupancy >= 60
      };
    });
  },

  // 7. Dự đoán khung giờ vàng (Golden Hour Prediction)
  getPredictedGoldenHours: async (facilityId = null, limit = 4) => {
    try {
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      
      let query = db('Bookings')
        .whereNotIn('status', ['Cancelled', 'Pending']) // Only count solid bookings
      ...
      const totalCourtsRes = await courtsQuery.count('id as count').first();
      const totalCourts = parseInt(totalCourtsRes.count || totalCourtsRes['count(*)'] || 1) || 1;

      // Use helper to calculate recommendations
      const recommendations = adminStatsService._calculateGoldenHours(bookings, totalCourts, 30);

      // Filter and Sort by highest occupancy
